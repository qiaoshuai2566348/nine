...
function deal(game) {
    ...
    let paiCount = 13 * game.conf.playerCount;
...
//检查是否可以弯杠(自己摸起来的时候),碰出来的三张，自己再接一张，就是弯杠
...
exports.begin = function (roomId) {
    ...
    for (var i = 0; i < roomInfo.conf.playerCount; ++i) {
        ...
        //玩家手上的牌的数目，用于快速判定碰杠
        //比如一万有几张，countMap[0]=4那就是杠
        //countMap:{0:4,1:3} = 一筒4张，二筒三张
        data.countMap = {};
        //玩家听牌，用于快速判定胡了的番数
        data.tingMap = {};
        data.pattern = "";
...
exports.peng = function (userId) {
    ...
    var i = game.turn;
    while (true) {
        var i = (i + 1) % game.conf.playerCount;
        if (i == game.turn) {
            break;
        } else {
            var ddd = game.gameSeats[i];
...
exports.hu = function (userId) {
    ...
    if (game.lastHuPaiSeat == -1) {
        ...
        var lp = (game.lastFangGangSeat - game.turn + game.conf.playerCount) % game.conf.playerCount;
        var cur = (seatData.seatIndex - game.turn + game.conf.playerCount) % game.conf.playerCount;
        if (cur > lp) {
